...
export async function GET() {
  ...
  const userGames = await getShuffleGames(userId);
  const rooms = await getRooms(userId);
  const roomId = rooms.length > 0 ? rooms[0].room_id : null;
        ...
        try {
          ...
          if (isRelevantMessage) {
            ...
            if (gameId !== null) {
              ...
              game = (await getShuffleGames(userId, gameId))[0];
            }
            const eventData = {
              type: isGameMessage ? 'game_update' : (isRoomMessage ? 'room_update' : 'user_update'),
              gameId: gameId,
